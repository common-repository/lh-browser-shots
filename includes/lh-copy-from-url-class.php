...
class LH_copy_from_url_class {
    ...
    static function maybe_include_libraries(){
    ...
    static function return_all_mimes(){
    ...
    static function mime2ext($mime){
        ...
        $all_mimes = self::return_all_mimes();
    ...
    static function get_existing_attachment_id($url){
	...
	*/
    ...
    static function maybe_strip_query_vars($url) {
	...
	*/
    ...
    static function guess_file_extension( $url, $response ){
        
        $mime_type = wp_remote_retrieve_header( $response, 'content-type' );
        
        if (!empty($mime_type )){
            
            $file_extension = self::mime2ext($mime_type);
        ...
        $file_extension = @image_type_to_extension( exif_imagetype( $url ) );
        ...
        $file_extension = '.'.pathinfo($filename_from_url['path'], PATHINFO_EXTENSION);
	...
	static function maybe_return_file_extension( $url, $response ){
	    
	    $file_extension = self::guess_file_extension( $url, $response );
	    $allowed_mimes = get_allowed_mime_types();
	    $lookup_string = substr($file_extension, 1);
	    
	    $check = false;
	    
	    
        foreach($allowed_mimes as $key => $value){
            
            if ($key == $lookup_string){
                
                $check = true;
                
            } elseif (str_contains($key, '|')){
                
                $array_check = explode('|', $key);
                
                if (in_array($lookup_string, $array_check )){
    ...
    */
    ...
    static function save_external_file($url, $post_id = 0, $desc = '', $check = false){
        
        self::maybe_include_libraries();

        $url = self::maybe_strip_query_vars($url);
        
        if (!self::isValidURL($url)){
            
            return new WP_Error( 'bad_url', __('the url supplied is bad.'));
            
        }

        if (!empty($check) and ($check === TRUE)){
        ...
        if (isset($existing) and is_numeric($existing)){
            ...
            $args = apply_filters(self::return_plugin_namespace().'_remote_get_args', $args);
            ...
            $file_extension = self::maybe_return_file_extension( $url, $response );

            if (empty($file_extension)){
                
                unlink( $tmpfname );
                return new WP_Error('hbad_file_extension', __('The file extension returned is invalid'));
			...
			$name = pathinfo( $url, PATHINFO_FILENAME )  . $file_extension;
